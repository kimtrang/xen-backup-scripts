...
def convert_yaml_to_json_file(data_dict, args):
    ...
    for platform in data_dict['platforms']:
        for host in platform['hosts']:
            json_dict[host['name']] = {'url': 'http://' + host['ip_addr'], 'username': host['user'], 'password': host['password'], 'repository': args.repository}
    write_json_data(json_dict)


def parse_args():
    parser = argparse.ArgumentParser(description="Convert YAML file to xenbackup's JSON config\n\n")
    parser.add_argument('--file', '-f', help='YAML File to convert', default='/etc/servers.yaml', required=True)
    parser.add_argument('--repository', '-r', help='Directory for backup', default='/builds/backups/xen', required=True)
    parser.add_argument('--json-output', '-o', help='JSON output file', default='/etc/xenbackup.json', required=True)
